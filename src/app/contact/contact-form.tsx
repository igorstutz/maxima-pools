...
// Where a successful submission lands. A page, not a dialog: it survives the
// back button, it can be shared or reopened, it gives the ad platforms a URL
// to count, and there is room on it for what someone actually needs after
// sending a request — when we call, from which numbers, what to read meanwhile.
...
export function ContactForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok && data?.ok) {
        ...
        setStatus("success");
        formEl.reset();
        // A client-side navigation, so the page never unloads and the beacons
        // analytics.lead() just queued still go out.
        router.push(THANK_YOU_PATH);
        return;
      }
  ...
  // The request is in and the browser is on its way to /thank-you/. This shows
  // for the instant that takes — never the empty form again, which would read
  // as "nothing happened" and invite a second submission.
  if (status === "success") {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
        <Loader2 size={28} className="animate-spin text-accent" />
        <p className="text-sm text-gray-500">Got it — taking you to the next steps…</p>
      </div>
    );
  }
