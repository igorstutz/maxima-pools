...
const icons: Record<string, LucideIcon> = {
  HandMetal, Layers, Gem, Ban, FlaskConical, Ruler, TriangleAlert, CircleAlert,
  ShieldCheck, Sparkles, Gauge, Snowflake, Droplets, Factory, Award, Shield,
};

export const metadata: Metadata = {
  alternates: { canonical: "/hand-laid-vs-combo-pools/" },
  title: content.seo.title,
  description: content.seo.description,
  openGraph: {
    url: "/hand-laid-vs-combo-pools/",
    ...
    <>
      ...
      <section className="relative overflow-hidden pt-28 pb-16 sm:pt-36 sm:pb-20 min-h-[520px] flex items-end">
        <HeroImageCycle
          ...
          alt="San Juan hand-laid fiberglass shell construction"
          interval={6000}
        />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <nav className="hero-animate hero-animate-1 flex items-center gap-2 text-sm text-white mb-8">
            ...
            <span className="text-white font-medium">Hand-Laid vs. Combo Pools</span>
          </nav>
      ...
      {/* ── The three methods — split layout ── */}
            ...
            <ScrollReveal direction="left">
              <div>
                ...
                <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-5">
                  {content.methods.headingLead}{" "}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">
                    {content.methods.headingHighlight}
                  </span>
                </h2>
                <p className="text-gray-600 text-lg leading-relaxed mb-6">
                  {content.methods.paragraph1}
                </p>
                <p className="text-gray-600 text-lg leading-relaxed mb-8">
                  {content.methods.paragraph2}
                </p>
                <div className="flex flex-wrap gap-2">
                  {content.methods.chips.map((item) => {
            ...
            <ScrollReveal direction="right">
              <div className="relative">
                ...
                <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl bg-white">
                  <ImageLightbox
                    src={content.methods.image}
                    alt={content.methods.imageAlt}
                    width={800}
                    height={1000}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="lg:hidden space-y-4">
            {content.comparison.rows.map((row, i) => (
              <ScrollReveal key={row.feature} delay={((i % 3) + 1) as 1 | 2 | 3}>
                <div className="bg-white/[0.04] border border-white/[0.06] rounded-2xl p-5">
                  <p className="text-white text-base font-semibold mb-4">{row.feature}</p>
                  <div className="space-y-3">
                    <div>
                      <p className="text-white/50 text-[11px] font-semibold uppercase tracking-wider mb-1">
                        {content.comparison.headerChopped}
                      </p>
                      <p className="text-white/70 text-sm">{row.chopped}</p>
                    </div>
                    <div>
                      <p className="text-white/50 text-[11px] font-semibold uppercase tracking-wider mb-1">
                        {content.comparison.headerCombo}
      ...
      <section className="py-16 sm:py-24 bg-gray-50 texture-noise">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <ScrollReveal direction="left">
              <div className="relative">
                ...
                <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl">
                  <Image
                    src={content.emptyPool.image}
                    alt={content.emptyPool.imageAlt}
                    width={900}
                    height={675}
            ...
            <ScrollReveal direction="right">
              <div>
                <div className="inline-flex items-center gap-2 bg-accent/10 rounded-full px-5 py-2 mb-6">
                  ...
                  <span className="text-accent font-semibold text-sm uppercase tracking-wider">
                    {content.emptyPool.badge}
                  </span>
                </div>
                <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-5">
                  {content.emptyPool.headingLead}{" "}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">
                    {content.emptyPool.headingHighlight}
                  </span>
                </h2>
                <p className="text-gray-600 text-lg leading-relaxed mb-5">
                  {content.emptyPool.paragraph1}
                </p>
                <p className="text-gray-600 text-lg leading-relaxed">
                  {content.emptyPool.paragraph2}
                </p>
              </div>
            </ScrollReveal>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-5 mb-10">
            {content.emptyPool.cards.map((card, i) => {
              const Icon = icons[card.icon];
              const isHandLaid = card.icon === "ShieldCheck";
              return (
                <ScrollReveal key={card.title} delay={((i + 1) as 1 | 2 | 3)}>
                  <div
                    className={`group rounded-2xl sm:rounded-3xl p-7 sm:p-8 h-full border transition-all duration-500 ${
                      isHandLaid
                        ? "bg-gradient-to-br from-accent/10 to-white border-accent/25 shadow-sm"
                        : "bg-white border-gray-100 hover:border-gray-200"
                    }`}
                  >
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center mb-5 border group-hover:scale-110 transition-transform duration-500 ${
                        isHandLaid
                    ...
                    >
                      {Icon && (
                        <Icon
                          size={24}
                          className={isHandLaid ? "text-accent" : "text-gray-400"}
                        />
                      )}
                    </div>
          ...
          {/* Safety note — keeps the claim aligned with the warranty terms */}
          <ScrollReveal>
            <div className="rounded-2xl sm:rounded-3xl border border-amber-200 bg-amber-50/70 p-6 sm:p-8">
              <div className="flex flex-col sm:flex-row gap-5">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900 mb-2">
                    {content.emptyPool.warningTitle}
                  </h3>
                  <p className="text-gray-600 leading-relaxed mb-4">
                    {content.emptyPool.warningText}
                  </p>
                  <Link
                    href={content.emptyPool.warningLinkHref}
                    className="group inline-flex items-center gap-2 text-sm font-semibold text-primary hover:text-accent transition-colors"
                  >
                    {content.emptyPool.warningLinkLabel}
      ...
      {/* ── What "premium" means on a combo brochure ── */}
      <section className="py-16 sm:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollReveal>
            <div className="max-w-3xl mb-14">
              <div className="inline-flex items-center gap-2 bg-accent/10 rounded-full px-5 py-2 mb-6">
                <div className="w-2 h-2 rounded-full bg-accent animate-pulse" />
                <span className="text-accent font-semibold text-sm uppercase tracking-wider">
                  {content.marketingSection.badge}
                </span>
              </div>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-5">
                {content.marketingSection.headingLead}{" "}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">
                  {content.marketingSection.headingHighlight}
                </span>
              </h2>
              <p className="text-gray-600 text-lg leading-relaxed">
                {content.marketingSection.intro}
              </p>
            </div>
          </ScrollReveal>
          ...
          <ScrollReveal>
            <div className="max-w-3xl mb-14">
              <div className="inline-flex items-center gap-2 bg-accent/10 rounded-full px-5 py-2 mb-6">
                <div className="w-2 h-2 rounded-full bg-accent animate-pulse" />
                <span className="text-accent font-semibold text-sm uppercase tracking-wider">
                  {content.durabilitySection.badge}
                </span>
              </div>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-5">
                {content.durabilitySection.headingLead}{" "}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">
                  {content.durabilitySection.headingHighlight}
                </span>
              </h2>
              <p className="text-gray-600 text-lg leading-relaxed">
                {content.durabilitySection.intro}
              </p>
            </div>
          </ScrollReveal>
